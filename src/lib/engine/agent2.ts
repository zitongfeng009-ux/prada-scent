...
import { EMOTION_LABEL, FAMILY_LABEL, SCENE_LABEL, WEATHER_EMOJI } from "../types";
...
function generateScentCombination(
  ...
  return `为你选择了 ${topFragrance.name}——${FAMILY_LABEL[topFragrance.family]}的精致结构。核心香材包含${notesList}。${tempAdvice}`;
}
...
function generateTriMapping(
  ...
  return {
    ...
    fragrance: {
      ...
      factors: [
        FAMILY_LABEL[topFragrance.family],
